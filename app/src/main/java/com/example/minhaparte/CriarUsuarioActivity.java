    ...
    private void salvarUsuario() {
        new Thread(() -> {
            try {
                ...
                json.put("nome", etNome.getText().toString().trim());
                json.put("matricula", etMatricula.getText().toString().trim());
                json.put("senha", etSenha.getText().toString().trim());
                json.put("tipo", tipo.name());
                json.put("cargo", etCargo.getText().toString().trim());
                json.put("setor", etSetor.getText().toString().trim());
                json.put("supervisor_nome", etSupervisor.getText().toString().trim());
                json.put("cpf", etCpf.getText().toString().trim());
                json.put("contato", etContato.getText().toString().trim());
                ...
                runOnUiThread(() -> {
                    if (responseCode == 201 || responseCode == 200) {
                        ...
                        Toast.makeText(this, "❌ Erro ao salvar usuário: " + responseCode, Toast.LENGTH_LONG).show();
                    }
                });
                conn.disconnect();
            } catch (Exception e) {
